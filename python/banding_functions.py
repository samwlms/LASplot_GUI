...
def main(operation, input, output, size, dpi):
    ...
    bands = generate_bands(operation, input_file, num_colour_bands)
...
def upper_limit(operation, input_file, divisions, layer):
    ...
    if operation == "gradient":
        metric = input_file.Z[input_file.Classification == 2]
    elif operation == "intensity":
        metric = input_file.intensity[input_file.Classification == 2]
    ...
    return int(np.amin(metric) + ((metric_delta / divisions) * layer))


def get_band(operation, input_file, divisions, layer):
    ...
    if operation == "gradient":
        ...
        metric = input_file.intensity

    # the derived upper bound for the given depth band
    upper_bound = upper_limit(operation, input_file, divisions, layer)
    ...
    if layer == 1:
        ...
        lower_bound = upper_limit(operation, input_file, divisions, (layer - 1))
...
def generate_bands(operation, input_file, bands):
    final_bands = ()
    for count in range(0, bands):
        current_band = get_band(operation, input_file, bands, count)
